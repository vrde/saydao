...
async function test() {
  const alice = await etherea.getWallet("http://localhost:8545");
  const bob = await etherea.getWallet("http://localhost:8545", 1);
  const carol = await etherea.getWallet("http://localhost:8545", 2);
  const mallory = await etherea.getWallet("http://localhost:8545", 3);
  ...
  const malloryInvite = await mallory.signMessage(etherea.to.array.uint16(101));
