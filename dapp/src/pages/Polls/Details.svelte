...
{#if $poll}
  {#if $poll.open}
    <div class="details">
      <p>
        <strong>{$poll.totalVotesPerc}%</strong> of {CONFIG.name} voted on this poll.
      ...
      <p>
        <em>Voting closes on <DateTime date={$poll.end} countdown={true} /></em>
      </p>
    </div>
  {:else}
    <div class="details">
  ...
  {:else}
    ...
    {#if $poll.open && $poll.hasTokens === null}
      <p>
        <strong>Note:</strong> you can't vote on this poll because you joined {CONFIG.name}
